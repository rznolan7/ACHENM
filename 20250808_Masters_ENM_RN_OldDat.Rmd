...
paleo_sp <- read_csv("D:/202508_MaxentModels/1_Occurrence_Data/original_occs.csv")
mod_sp <- read_csv("D:/202508_MaxentModels/1_Occurrence_Data/mod_occs.csv")

env_sp <- rbind(paleo_sp, mod_sp)
...
#plot in a quick map
cn <- rspat::spat_data("pt_countries")
...
all_occ_thinned <- all_occ_thinned %>%
  sf::st_drop_geometry()  #remove the geometry column that was created when we did the near calc
...
#plio_occ <- subset(all_occ_thinned, Time == "Plio")
EP_occ <- subset(all_occ_thinned, Time == "EPleist")
MP_occ <- subset(all_occ_thinned, Time =="MPleist")
LP_occ <- subset(all_occ_thinned, Time == "LPleist")
H_occ <- subset(all_occ_thinned, Time == "Holo")
Mod_occ <- subset(all_occ_thinned, Time == "Modern")
```

```{r}  
#separate out time bins to run individual models - old data
EP_occ <- subset(all_occ_thinned, Time == "EPleist")
MP_occ <- subset(all_occ_thinned, Time =="MPleist")
LP_occ <- subset(all_occ_thinned, Time == "LPleist")
H_occ <- subset(all_occ_thinned, Time == "Holo")
...
master.background <- do.call('rbind', list(EP.back, MP.back, LP.back, H.back, Mod.back) )
#master.background <- do.call('rbind', list(EP.back, MP.back, LP.back, H.back) ) #old data
#set extent during model run

#bind together time bins of occurrences
master.occs <- rbind(EP.occs.df, MP.occs.df, LP.occs.df, H.occs.df, M.occs.df)
#master.occs <- rbind(EP.occs.df, MP.occs.df, LP.occs.df, H.occs.df)
```
